...
import org.onehippo.devx.webservices.webfiles.WebFileUpdater;
...
public class PropertiesResource {
    ...
    public Response updatePropertyByPath(@ApiParam(required = true, value = "Path of the property to update e.g. '/content/documents/hippostd:foldertypes'")
        ...
        try {
            ...
            session.save();
            if(parentPath.startsWith("webfiles")) {
                String template = jcrProperty.getValues().get(0);
                WebFileUpdater.update(session, node, template);
            }
        } catch (Exception e) {
            log.error("Error: {}", e);
            throw new WebApplicationException(e);

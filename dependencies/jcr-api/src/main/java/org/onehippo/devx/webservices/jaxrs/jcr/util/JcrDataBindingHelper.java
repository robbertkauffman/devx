...
package org.onehippo.devx.webservices.jaxrs.jcr.util;
...
import org.onehippo.devx.webservices.jaxrs.jcr.model.JcrNode;
import org.onehippo.devx.webservices.jaxrs.jcr.model.JcrProperty;
...
public final class JcrDataBindingHelper {
    ...
    public static JcrProperty getPropertyRepresentation(Property property) throws RepositoryException {
        ...
        if (property.isMultiple()) {
            for (Value propertyValue : property.getValues()) {
                values.add(getPropertyValueAsString(propertyValue, session));
            }
        } else {
            values.add(getPropertyValueAsString(property.getValue(), session));
        }
        data.setValues(values);
        return data;
    }

    public static String getPropertyValueAsString(final Value value, final Session session) throws RepositoryException {
        StringWriter stringWriter = new StringWriter();
        if(value.getType() == PropertyType.BINARY) {
            try {
                return ValueHelper.convert(value, PropertyType.STRING, session.getValueFactory()).getString();
            } catch (ValueFormatException e) {
                log.error("An exception occurred while trying to convert the value: {}", e);
            }
        } else {
     ...
     * @param mixins a {@link List} of mixins that need to be applied.
     * @throws RepositoryException
     */
     ...
     * @param jcrProperties a {@link List} of properties
     * @throws RepositoryException
     */
     ...
     * @param node the {@link Node} to which to add the child nodes to.
     * @param nodes a {@link List} of child nodes.
     * @throws RepositoryException
     */
     ...
     * @param node the {@link Node} on which to set the property
     * @param property the property
     * @throws RepositoryException
     */
